...
# --- Load CSV ---
def Preprocessing_ev(csv_path="month_main_table.csv"):
    ...
    # --- Compute EV per RTO ---
    rto_EVs = (
        df.groupby(["District", "RTO_CLEAN"])["EV"]
        .sum()
        .reset_index()
        .rename(columns={"EV": "EV"})
    )

    # --- Compute EVs for each brand ---
    brand_dfs = []
    for brand_name, oem_list in brand_groups.items():
        temp = (
            df[df["OEM_UPPER"].isin(oem_list)]
            .groupby(["District", "RTO_CLEAN"])["EV"]
            .sum()
            .reset_index()
            .rename(columns={"EV": brand_name})
        )
    ...
    merged["MS"] = (merged["Tata Motors"] / merged["EV"] * 100).round(2)
    final_cols = [
        "RTO_CLEAN", "District",
        ...
        "BYD INDIA PRIVATE LIMITED", "EV", "MS","Rank_Tata"
    ]
    ...
    res = supabase.table("rto_ev").upsert(payload).execute()
